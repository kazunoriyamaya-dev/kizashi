...
**Phase 0 - Phase 15 全完了 / 本番デプロイ一歩手前**

最終チェックリストは [`docs/運用_デプロイ前最終チェック.md`](docs/運用_デプロイ前最終チェック.md) を参照。
`pnpm lint` / `pnpm build` / `pnpm test` (62 ケース) すべて通過済み。
...
- **TypeScript / TSX**: 220+ ファイル
- **SQL マイグレーション**: 21 ファイル
- **DB テーブル**: 49 テーブル (基幹 27 + マーケ 22、全 RLS 適用済)
- **PL/pgSQL 関数**: 17 関数 (`security definer` + `revoke from public`)
- **API Route**: 43 ハンドラ
- **画面**: 51 画面 (admin 28 含むマーケ 11 / instructor 8 / customer 15 / 公開 LP・ブログ・トップ)
- **通知テンプレート**: 12 種類 × 5 チャネル + ステップメール (任意ステップ数)
- **Vercel Cron**: 6 ジョブ (基幹 4 + マーケ 2)
- **単体テスト**: 10 スイート 62 ケース (Vitest)
